    ...
    : mpImuSensor(imu_sensor), mpCamera{left_cam, right_cam}
{
    ...
    mTij[I_IMU][I_LC] = mpImuSensor->Tbs.inverse() * mpCamera[I_LC]->Tbs;
    mTij[I_IMU][I_RC] = mpImuSensor->Tbs.inverse() * mpCamera[I_RC]->Tbs;
    mTij[I_LC][I_IMU] = mTij[I_IMU][I_LC].inverse();
    mTij[I_RC][I_IMU] = mTij[I_IMU][I_RC].inverse();
    mTij[I_LC][I_RC] = mpCamera[I_LC]->Tbs.inverse() * mpCamera[I_RC]->Tbs;
