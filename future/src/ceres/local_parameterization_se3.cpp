...
bool LocalParameterizationSE3::Plus(const double* x, const double* delta, double* x_plus_delta) const {
    Eigen::Map<const Eigen::Vector3d> p(x), dp(delta), omega(delta + 3);
    Eigen::Map<const Sophus::SO3d> q(x + 3);
    Eigen::Map<Eigen::Vector3d> p_(x_plus_delta);
    Eigen::Map<Sophus::SO3d> q_(x_plus_delta + 3);
