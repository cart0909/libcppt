...
IMUPreintegrator::IMUPreintegrator(const Eigen::Matrix3d& gyr_noise_cov_,
                                   const Eigen::Matrix3d& acc_noise_cov_)
    : gyr_noise_cov(gyr_noise_cov_), acc_noise_cov(acc_noise_cov_)
{
...
IMUPreintegrator::IMUPreintegrator(const IMUPreintegrator& pre):
    ...
    gyr_noise_cov(pre.gyr_noise_cov),
    acc_noise_cov(pre.acc_noise_cov)
{
...
void IMUPreintegrator::update(const Vector3d& omega, const Vector3d& acc, const double& dt)
{
    ...
    _cov_P_V_Phi = A*_cov_P_V_Phi*A.transpose() +
        Bg*gyr_noise_cov*Bg.transpose() +
        Ca*acc_noise_cov*Ca.transpose();
