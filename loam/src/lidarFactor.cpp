...
bool LidarLineFactorNeedDistort::Evaluate(double const * const* parameters_raw,
    ...
    Eigen::Map<const Sophus::SO3d> wmap_q_lidarj(parameters_raw[0]);
    Eigen::Map<const Eigen::Vector3d> wmap_p_lidarj(parameters_raw[0] + 4);
    ...
    if(jacobians_raw){
        ...
        if(jacobians_raw[0]) {
            ...
            I.setIdentity();
            J.leftCols<3>() = I;
            J.rightCols<3>() = wmap_q_lidarj.matrix() * -Sophus::SO3d::hat(curr_point);
